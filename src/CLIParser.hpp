...
public:
  ...
  bool hasFlag(const std::string &flag)
      const; // true if a certain flag(argument) exists in argv

  int getIndex(const std::string &arg)
      const; // returns the index of an argument in argv (-1 if argument doesn't
  ...
  std::string getValue(const std::string &arg)
      const; // returns value after flag, for example you can find filename
